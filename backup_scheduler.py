    ...
    def schedule_backup(self, cr, user, context={}):
        ...
        for rec in confs:
            ...
            if rec.name in db_list:
                ...
                conn = xmlrpclib.ServerProxy(uri + '/xmlrpc/db')
                bkp=''
                _logger.info('testing1111..')
                try:
                    bkp = execute(conn, 'dump', tools.config['admin_passwd'], rec.name)
                except:
                    _logger.notifyChannel('backup', netsvc.LOG_INFO, "Couldn't backup database %s. Bad database administrator password for server running at http://%s:%s" %(rec.name, rec.host, rec.port))
                    continue
                _logger.info('testing2222..')
                fp = open(file_path,'wb')
                _logger.info('testing3333..')
                bkp = base64.decodestring(bkp)
                _logger.info('testing4444..')
                fp.write(bkp)
                _logger.info('testing5555..')
                fp.close()
            else:
